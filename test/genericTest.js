const createSentryMiddleware = require("../src/main");
...
function testSentryForRaven(Sentry) {
  ...
  describe("in the default configuration", () => {
    ...
    it("if explicitly passed extras contain a `state` property, the middleware version wins", async () => {
    ...
    it("if explicitly passed extras contain a `lastAction` property, the middleware version wins", async () => {
      Sentry.setExtras({
        anotherValue: 10,
        lastAction: "SOME OTHER LAST ACTION"
      });
      expect(context.store.dispatch({ type: "INCREMENT" }));
      Sentry.captureException(new Error("Crash!"));
      await Sentry.flush();
      const { extra } = sendEvent.mock.calls[0][0];
      expect(extra).toMatchObject({
  ...
  describe("with all the options enabled", () => {
    beforeEach(() => {
      ...
      context.getUserContext = jest.fn(state => ({
        name: `user context ${state.value}`
      }));
      context.getTags = jest.fn(state => ({ tagKey: `tags ${state.value}` }));
    ...
    it("transforms the user context on data callback", () => {
      ...
      expect(sendEvent.mock.calls[0][0].user).toEqual({
        name: "user context 1",
        userId: 1,
        username: "captbaritone"
      });
    });
    it("transforms the tags on data callback", () => {
      ...
      expect(sendEvent).toHaveBeenCalledTimes(1);
      expect(sendEvent.mock.calls[0][0].tags).toEqual({ tagKey: "tags 1" });
    });
  });
  ...
  describe("Middleware is attached to the scope", () => {
    ...
    it("so (sadly) multiple middleware's created in the same scope will collide (the last one wins)", async () => {
      ...
      expect(reportedState).toEqual(storeTwo.getState());
      expect(reportedState).not.toEqual(storeOne.getState());
    });
  });
}
